...
export interface DistributionPanelMeterStatus {
...
export class DistributionPanelMeterDevice implements IBaseDevice {
  ...
  private _status: DistributionPanelMeterStatus = {
    operationStatus: "on",
    faultStatus: "noFault",
  ...
  get status(): DistributionPanelMeterStatus {
    return { ...this._status };
  }
  ...
  setStatus(newStatus: Partial<DistributionPanelMeterStatus>): void {
    let changed = false;
